...
namespace LeonsWorld 
{
    public class GameManager: MonoBehaviour 
    {
        ...
        private float attackRate = 0.0 f;
        private float attackFlame = 0.02 f;
        private float canFireAfter = 0.0 f;
        private float nerf = 1 f;
        ...
        void AerosolCan() 
	{
            if (lookRight) 
	    {
                if (bulletDamage == 1) 
		{
                    Rigidbody bPrefab1 = Instantiate(bulletPrefab, new Vector2(transform.position.x - 0.50 f, transform.position.y - 0.24 f), Quaternion.identity) as Rigidbody;
        ...
        void NerfGun() 
	{
            if (lookRight) 
	    {
                if (bulletDamage == 5) 
		{
                    Rigidbody bPrefab2 = Instantiate(bulletPrefab2, new Vector2(transform.position.x - 0.22 f, transform.position.y - 0.33 f), Quaternion.identity) as Rigidbody;
        ...
        void FlameThrower() 
	{
            if (lookRight) 
	    {
                if (bulletDamage == 6) 
		{
                    Rigidbody bPrefab3 = Instantiate(bulletPrefab3, new Vector2(transform.position.x + 0.38 f, transform.position.y - 0.26 f), Quaternion.identity) as Rigidbody;
	...
	{
            yield return new WaitForSeconds(0.35 f);
            weaponBarSlider.value += 1 f;
        }
	...
	{
            yield return new WaitForSecondsRealtime(5 f);
            RestartScene();
        }
